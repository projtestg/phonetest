...
function displayCafeInfo(index) {
	var len, i, name="", type="", score="", reqion="", phone="", address="", memo="", pic="";
	...
	if(myCafeRecord.name != null) {		// 맛집 이름
		name = '<div class="ui-bar-a"><h3>' + myCafeRecord.name + '</h3></div>';
	} else {
		name = '<p>이름 : 정보없음</p>';
	}
	...
	if(myCafeRecord.reqion != null) {		// 지역
		reqion = '<p>지역 : ' + myCafeRecord.reqion + '</p>';
	} else {
		reqion = '<p>지역 : 정보없음</p>';
	}
	if(myCafeRecord.phone != null) {		// 전화
	...
	$('#cafeInfoArea').html(name + type + score + reqion + phone + address + memo);
	$.movile.chagePage("#cafeInfoShowPage", "slide", false, true);
}
...
function getCafeRouteMap() {
	...
	navigator.geolocation.getCurrentPosition(
		function(varPosition) {
			...
			var start =  new google.maps.latLng(lag, lng);
			var end = recordSet.rows.item(varPosition).address;
			var mode = 'TRANSIT'
			var request = {
				origin:start,
				destination:end,
				travelMode: eval('google.maps.DirectionsTravelMode.' + mdoe)
			};
	...
	function(posError) {
		alet('Error Code : ' + posError + ' / Error Message : ' + posError.Message)
	}, { maximumAge : 3000, timeout: 50000, enavleHighAccuracy: true}
	);
}
